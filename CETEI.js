var CETEI = (function () {
  ...
  var defaultBehaviors = {
    ...
    "tei": {
	  ...
	  /*
	  "w": [
	    [":not([join=right])", function(elt){
		  //elt.innerText += " ";
		  elt.insertAdjacentHTML("afterend", " ");
		  / *
		  if(elt.nextSibling)
		      elt.parentNode.insertBefore(" ", elt.nextSibling);
		  else
  ...
  function copyAndReset(node) {
    let clone = (n) => {
      ...
      for (let nd of Array.from(n.childNodes)){
		...
		{
			console.log(nd.nodeValue);
			if(!/^\s+$/.test(nd.nodeValue))
    ...
    domToHTML5(XML_dom, callback, perElementFn){
      ...
      let convertEl = (el) => {
        ...
        for (let att of Array.from(el.attributes)) {
			...
			/* MISKO */
			if(att.name == "pos" || att.name == "lemma") {
			  sAttrTitle = `${sAttrTitle}- sr${att.name}=${att.value}\n`;
			}
            if (att.name == "xml:id") {
              newElement.setAttribute("id", att.value);
			  if (el.localName == "w")
				  sAttrTitle = `${sAttrTitle}- id=${att.value}\n`;
            }
		    /* END MISKO */
        }
		/* MISKO */
		if(el.localName == "w")
		{
			newElement.setAttribute("data-title", sAttrTitle);
		}
		/* END MISKO */
        ...
        for (let node of Array.from(el.childNodes)) {
            if (node.nodeType == Node.ELEMENT_NODE) {
				...
				if (node.nodeName == 'rs' || (node.hasAttribute('join') && node.getAttribute('join') == 'right'))
				    joinRight = true;

            }
			/* MISKO */
            else {
				let isWs = /^\s+$/.test(node.nodeValue);
				if(!isWs || (isWs && !joinRight))
